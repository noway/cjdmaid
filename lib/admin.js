...
function Udp(host, port) {
	...
	that.send = function (msg) {
		...
		that.client.send(
			...
			function (err, bytes) {
				if (err) {
					console.error("Error while sending (" + bytes + "b):");
					console.error(err);
				}
			}
		);
...
function Admin() {
	...
	that._receiverThread = function () {
		...
		.then(function (data) {
			...
			if (benc.txid === "keepaliv") {
				...
				else if (typeof that.streams[benc.streamId] !== "undefined") {
	...
	that.callFunc = function (funcName, args) {
		args = args || {};
		var txid;
		var cookie;
