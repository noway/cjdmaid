...
config.readCjdmaidConf = function () {
	...
	.then(function (cjdmaidJson) {
		...
		if (!util.isDef(cjdmaidJson.cjdrouteConf)) {
			err = new Error("field cjdrouteConf not found in config");
			return util.panic(err);
		}
		...
		if (!util.isDef(cjdmaidJson.cjdnsadminConf)) {
			err = new Error("field cjdnsadminConf not found in config");
			return util.panic(err);
		}
